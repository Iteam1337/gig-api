...
async function indexElasticDocument (doc, { id }, method = 'index') {
...
async function getElasticIdFromSQLDocument (doc) {
  const query = elasticQuery
    .bodybuilder({
      from: 0,
      size: 1
    })
    .query('term', 'source', doc.source)
    .query('term', 'source_id', doc.source_id)
    .build()

  const { hits: { hits } } = await elasticClient.search(query)

  if (!hits.length) {
    return null
  }

  const [{ _id: elasticId }] = hits
...
async function update (job) {
  ...
  if (!dbDocument) {
    console.error(`could not find db document {"source_id":"${job.source_id}","source":"${job.source}"}`)
    return null
  }
  ...
  if (!upsertResult || upsertResult.id !== dbDocument.id) {
    console.error(`could not run upsert for {"source_id":"${toSave.source_id}","source":"${toSave.source}"}`)
    return null
  }

  await indexElasticDocument(toSave, { id: dbDocument.id }, 'update')

  if (toSave.source === 'gigstr' && toSave.source_id === '1323') {
    console.log(toSave)
  }
  return upsertResult
}
...
async function insert (job) {
  ...
  await indexElasticDocument(snakeCasedDoc, result)
