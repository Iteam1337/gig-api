...
function validateInput (query = {}) {
  function checkEnum (string = '', list = [], defaultValue = null) {
...
async function insertJob (job) {
  ...
  return db.insert('jobs', snakecaseKeys({
    ...
    paymentType,
    currency,
    experience: handleLtree(experience),
    skills: handleLtree(skills),
    education: handleLtree(education),
    languageSkills: handleLtree(languageSkills),
    startDate,
    endDate,
    listedDate,
    source,
...
async function insertJobs (jobs) {
  const results = await Promise.all((Array.isArray(jobs) ? jobs : [jobs]).map(insertJob))
