...
function getJobsSql ({ sort, latLongSearch, orderBy, experience, career, education, language, languageSkills, skills }) {
  function getScore ({ defaultValue = 'created_at', computed = [] } = {}) {
    computed = Array.isArray(computed) ? computed : []

    if (experience) { // Yrkeserfarenhet
      computed.push({ order: `experience ? '{${experience.join(',')}}'`, as: 'experience_score' })
    }
    ...
    if (!computed.length) {
      computed.push({ orderBy: defaultValue})
    }
    ...
    return computed.reduce((array, { score, orderBy }) => {
      ...
      if (score) {
        ...
        array[1] = `${scoreString}\n  , CASE WHEN (${score}) THEN 0 ELSE 1 END AS ${orderBy}`
      }

      if (orderBy) {
        array[0] = orderBy ? `${orderByString} + ${orderBy}` : `${orderBy}`
      }
...
;WITH computed AS (
  SELECT
    *${latLongSearch ? `
  , distance
  , RANK() OVER (ORDER BY distance) AS distance_score` : ''}${score}
  FROM
    jobs${latLongSearch ? `
...
async function getJobs (query) {
  ...
  console.log(sqlParams.reduce((string, key, index) => string.replace(new RegExp(`\\$${index + 1}`, 'g'), `'${key}'`), sql))
